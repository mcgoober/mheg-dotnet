...
namespace MHEG
{
    class MHFontBody
    {
        protected MHOctetString m_DirFont;
        protected MHObjectRef m_IndirFont;
        ...
        {
            if (p.NodeType == MHParseNode.PNString) p.GetStringValue(m_DirFont);
            else m_IndirFont.Initialise(p, engine);
        }

        public void Print(TextWriter writer, int nTabs)
        {
            if (m_DirFont.Size > 0) m_DirFont.Print(writer, nTabs);
            else m_IndirFont.Print(writer, nTabs);
        }

        public bool IsSet() 
        {
            return m_DirFont.Size != 0 || m_IndirFont.IsSet(); 
        }
