...
{
    abstract class MHActionInt: MHElemAction
    {
        protected MHGenericInteger m_Argument;

        public MHActionInt(string name)
            : base(name)
        {
            m_Argument = new MHGenericInteger();
        }

        public abstract void CallAction(MHEngine engine, MHRoot pTarget, int nArg);
